...
$diff_1 = date_diff($date_3,$date_1);
  ...
  $cst       =  mysql_query("SELECT price FROM room_price WHERE name = 'room_premium_deluxe'");
  $rw_cst    =  mysql_fetch_array($cst);
  $room_cost =  $rw_cst['price'];


  $result    = mysql_query("SELECT * FROM room_premium_deluxe WHERE check_out_date<'$check_in'" ) or 
  ...
  if(mysql_num_rows($result)>=$number && $ff>=$number)
    { 
       ?>  <div class="lged"> <h2>Congratz!! Room is Available</h2><p></p> <?php 
         ...
         <form action = "process_premium_deluxe_done.php" method="POST">
