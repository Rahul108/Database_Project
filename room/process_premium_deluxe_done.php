...
<?php 
      ...
      if($row_login['id'] == $user && $row_login['password'] == $pass)
      {
        ...
        for($x=200;$x<=210;$x++)
        {
            ...
            $check = mysql_query("SELECT * FROM room_premium_deluxe WHERE room_no = '$x' AND check_out_date < '$check_in' OR check_in_date>'$check_out' ") ;
            ...
            if($row_check['room_no'])
            {
              ...
              else
              {
                ...
                {
                  mysql_query("UPDATE room_premium_deluxe SET id = '$user' , check_in_date = '$check_in' , check_out_date = '$check_out' WHERE room_no = '$x' AND check_out_date < '$check_in' ");
                }
              }
            } 
